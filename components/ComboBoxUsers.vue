<template>
  <Combobox as="div" v-model="selected" @update:modelValue="check">
      <div class="relative mt-3">
        ...
        >
          <ComboboxOptions
            class="cb-op absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm"
          >
            <div
...
// const group = toRef(props.options)
...
const filteredGroup = computed(() =>
  ...
  function disable(id) {
    const rId = props.items.filter((i) => i.user_id === id).map((r) => r.role)
    return rId.length > 0 
      ? true 
      : false
  }
  function roleId(id) {
    // const rId = props.items.filter((i) => i.user_id === id).map((r) => r.role)
    // props.items.filter((i) => i.user_id === id)
    //   ? role.filter((i) => i.id === rId).map((t) => t.title)
    //   : "";
    const rId = props.items.filter((i) => i.user_id === id).map((r) => r.role)[0];
    const rTitle = role.filter((i) => i.id === rId).map((t) => t.title);
    return rTitle.length > 0
      ? `(${rTitle[0]})`
      : ``
    // return props.items.filter((i) => i.user_id === id).map((r) => r.role).toString()
  }
