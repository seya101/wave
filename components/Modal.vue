          ...
          <div class="relative bg-white rounded-lg shadow">
            ...
            <div class="py-6 px-6 lg:px-8">
              ...
              <form class="space-y-6">
                ...
                <ComboBoxUsers :items="items" :options="users" :placeholder="`User`" :header="`User`" @selected="cbName" />
                <ComboBox :options="role" :placeholder="`Role`" :header="`Role`" @selected="cbRole" />
                <Checkbox :options="brands" :placeholder="`Brands`" :header="`Brands`" @selected="cbBrands" />
...
export default {
  ...
  methods: {
    ...
    saveUser() {
      this.$emit('submit', [{"group_id": this.cbTeam, "user_id": this.cbName, "brands": this.cbBrands, "role": this.cbRole}])
    }
  }
}
</script>